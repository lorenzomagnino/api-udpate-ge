...
# Import GCS manager for cloud storage operations
import sys
...
class VeytAPIClient:
    def __init__(
        ...
        self.token_file = Path("veyt/.veyt_token_cache.json")
        self._token = None
...
def save_to_csv(df: pd.DataFrame, filename: str, output_dir: str = "veyt_data"):
    ...
    # Check if output_dir is a GCS path
    if output_dir.startswith("gs://"):
        # Extract bucket and path from gs://bucket/path format
        parts = output_dir.replace("gs://", "").split("/", 1)
        bucket_name = parts[0]
        ...
        # Use gcs_manager to upload
        success = gcs_manager.upload_dataframe(df, gcs_path, index=False)
        if success:
            logging.info(f"Saved data to gs://{bucket_name}/{gcs_path}")
            return f"gs://{bucket_name}/{gcs_path}"
        else:
            logging.error(f"Failed to save data to gs://{bucket_name}/{gcs_path}")
            return None
    else:
        # Local file system
