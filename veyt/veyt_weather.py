...
from config import WEATHER, DEFAULT_FROM_DATE, DEFAULT_UNTIL_DATE, DEFAULT_OUTPUT_DIR
...
def get_weather_data(client, from_date=DEFAULT_FROM_DATE, until_date=DEFAULT_UNTIL_DATE, output_dir=DEFAULT_OUTPUT_DIR):
    ...
    for weather_type, curve_id in WEATHER.items():
        ...
        if df is not None and not df.empty:
            ...
            print(f"Retrieved {len(formatted_df)} {weather_type.upper()} records")
            ...
            print(f"No {weather_type.upper()} data retrieved")
...
def create_combined_weather_data(weather_data, output_dir=DEFAULT_OUTPUT_DIR):
    ...
    if hdd_data is not None and cdd_data is not None and not hdd_data.empty and not cdd_data.empty:
        ...
        print(f"Daily aggregate data has {len(daily_aggregate)} records")
        print("Sample daily aggregate:")
        print(daily_aggregate.head(3))
        ...
        print("Cannot create combined data - missing HDD or CDD data")
